...
from utils import make_plot, sub_branches, are_roots
...
for i, sb in enumerate(sub_branches(points)):
    choices = ['black', 'red']
    colors = [choices[int(i)] for i in are_roots(sb)]
    make_plot(sb, primes, currentdir + '/' + str(i), ratios=False, origin=True,
              colors=colors, dot_size=2)
